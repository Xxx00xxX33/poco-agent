...
- [ ] schema 能校验必填 id、trigger_type 和 source_actor
- [ ] schema 能序列化为 JSON，用于 run/message metadata
- [ ] 无效 trigger_type 被拒绝
...
- [ ] 新 run 能在 config snapshot 中保存 trigger_context
- [ ] agent user message 能携带 trigger_context metadata
- [ ] 旧 message 没有 metadata 时前端仍按普通消息渲染
...
- [ ] trigger body 优先读取 `content.text`，不使用截断 preview 作为正文
- [ ] envelope 包含 trigger message id 和 thread root id
- [ ] recent context 不再作为唯一上下文来源
...
- [ ] channel mention 触发时生成 `trigger_type=channel_mention`
- [ ] agent DM 触发时生成 `trigger_type=agent_dm`
- [ ] Task enqueue 的 prompt 不再必须等于完整 composed context
...
- [ ] 新 agent message content 同时包含 text block 和 metadata.trigger_context
- [ ] text_preview 来自 trigger body，而不是系统 composed prompt
- [ ] 旧普通 chat enqueue 不受影响
