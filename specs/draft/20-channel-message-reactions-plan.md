...
| --- | --- |
| **创建日期** | 2026-05-07 |
| **预期改动范围** | backend server channel message reaction model/API/internal runtime API / executor channel reaction MCP tool injection / executor-manager proxy / frontend conversation message UI and i18n / backend and executor tests |
| **改动类型** | feat |
| **优先级** | P1 |
| **状态** | drafting |
...
- 当前 agent-facing channel tools 已有两条先例：`executor/app/core/channel_tasks.py` 和 `executor/app/core/channel_artifacts.py`。它们都只在 `TaskConfig` 具备 `server_id`、`channel_id`、`agent_identity_id` 时由 executor 注入，并通过 executor-manager 代理到 backend internal API。
...
#### 3. agent 贴表情应走内置 MCP tool，而不是普通消息协议

agent 需要的是“对某条消息执行 reaction 操作”的结构化能力。最贴合现有架构的方式是新增 channel-scoped built-in MCP server，例如 `__poco_channel_reactions`，提供 `add_channel_message_reaction` / `remove_channel_message_reaction`。tool 调用自动绑定当前 session 的 server/channel/agent identity，上游模型只传 `message_id` 和 `emoji`。
...
- [ ] reaction 不存入 `server_channel_messages.content`
- [ ] 同一 actor 对同一消息的同一 emoji 只能存在一条记录
- [ ] 删除消息会级联删除 reaction
...
- [ ] API 会拒绝空 emoji 和明显非法输入
- [ ] 同一个 emoji 序列在后端有稳定存储形式
...
- [ ] 用户不能给无权访问的频道消息贴 reaction
- [ ] agent 不能对非当前 channel 的消息贴 reaction
- [ ] agent tool 不接受 actor 身份参数
...
- [ ] add reaction 幂等，重复添加不会增加 count
- [ ] remove reaction 幂等，不存在时返回成功或明确 no-op
- [ ] 聚合查询支持一次传入多条 message id
...
- [ ] 频道根消息列表返回 reaction groups
- [ ] thread root 和 replies 返回 reaction groups
- [ ] 当前用户自己的 reaction 有布尔标记，前端无需重复推导
...
- [ ] 公开 API 可添加当前用户 reaction
- [ ] 公开 API 可取消当前用户 reaction
- [ ] 目标 message 不在当前 channel 时返回 not found 或 bad request
...
#### 2.1 新增 backend internal reaction API

**描述：** 新增 executor-manager 可调用的 internal API，通过 `session_id` 解析当前 agent runtime scope，并调用 reaction service：

- `POST /internal/channel-message-reactions/add?session_id=...`
- `POST /internal/channel-message-reactions/remove?session_id=...`
...
- `backend/app/api/v1/internal_channel_message_reactions.py`
...
- [ ] internal API 只接受 internal token
- [ ] actor identity 来自 session snapshot，不来自 body
- [ ] 目标 message 必须属于当前 channel

#### 2.2 新增 executor-manager 代理入口

**描述：** 按 `agent_channel_artifacts` 和 `agent_channel_tasks` 的方式，在 executor-manager 增加代理路由，把 executor tool 请求转发到 backend internal API。

建议路径：

- `POST /api/v1/agent-channel-reactions/add`
- `POST /api/v1/agent-channel-reactions/remove`

**涉及文件：**

- `executor_manager/app/api/v1/agent_channel_reactions.py`
- `executor_manager/app/api/v1/__init__.py`
- `executor_manager/app/services/backend_client.py`

**验收标准：**

- [ ] 代理请求自动带 internal token 到 backend
- [ ] trace headers 按现有 `_trace_headers` 模式透传

#### 2.3 注入 channel reaction MCP tools

**描述：** 在 executor 新增 `ChannelReactionClient` 和 built-in MCP server。建议工具：
...
- `executor/app/core/channel_reactions.py`
...
- [ ] channel-scoped run 自动注入 reaction MCP server
- [ ] 非 channel run 不注入 reaction tools
- [ ] tool 参数不包含 server/channel/agent 身份
...
在频道消息列表、搜索/inbox 列表和 thread drawer 中展示 reaction groups，并支持用户点击添加或取消常用 emoji。
...
#### 3.2 在 MessageRow 渲染 reaction bar
...
- hover toolbar 增加 emoji button 打开小型常用 emoji picker

第一版 emoji picker 可以先使用固定常用集合，例如 `👍`、`✅`、`👀`、`❤️`、`🎉`、`🚀`。所有按钮 aria-label 和 tooltip 文案走 i18n。
...
- `AgentExecutor` 只在 channel scope 注入 reaction MCP
...
这次改动的核心问题是：频道消息需要轻量反馈机制，而 reaction 是独立的多人/多 agent 关系，不应混进消息正文。实现上建议新增 `server_channel_message_reactions` 表来表达 `message x emoji x actor`，在人类 API 中按当前用户落库，在 agent runtime 中通过 channel-scoped MCP tool 以当前 agent 身份落库。消息读取接口返回聚合后的 reaction groups，前端在 `MessageRow` 里做展示、点击切换和乐观更新。
